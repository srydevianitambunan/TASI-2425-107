...
class CKE(KnowledgeRecommender):
    ...
    def evaluate_long_tail(self, train_data, topk_items):
        # Hit rate di antara top-k
        item_counter = Counter(train_data.inter_feat['item_id'].tolist())
        long_tail_items = set([item for item, count in item_counter.items() if count <= 2])
        hits = sum(1 for item in topk_items if item in long_tail_items)
        return hits / len(topk_items)

    def compute_sparsity(self, dataset):
        n_users = dataset.user_num
        n_items = dataset.item_num
        n_interactions = dataset.inter_feat.shape[0]
